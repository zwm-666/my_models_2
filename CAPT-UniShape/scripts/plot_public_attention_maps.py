...
DEFAULT_OP_CHANNEL_LABELS = "stack_voltage,stack_current,stack_power"
DEFAULT_EIS_CHANNEL_LABELS = "response,gradient,cumulative,freq_axis"


def _parse_label_csv(value: str, expected_count: int) -> list[str]:
    labels = [item.strip() for item in str(value).split(",") if item.strip()]
    if len(labels) < expected_count:
        labels.extend(f"channel_{idx}" for idx in range(len(labels), expected_count))
...
def run_attention_plot(
    ...
    output_dir: Path,
    split: str = "test",
    output_prefix: str = "public_ac_voltage",
    title_prefix: str = "Public AC Voltage",
    op_channel_labels: str = DEFAULT_OP_CHANNEL_LABELS,
    eis_channel_labels: str = DEFAULT_EIS_CHANNEL_LABELS,
) -> dict[str, Path]:
    ...
    op_labels = _parse_label_csv(op_channel_labels, op_summary["matrix"].shape[1])
    eis_labels = _parse_label_csv(eis_channel_labels, eis_summary["matrix"].shape[1])

    output_dir.mkdir(parents=True, exist_ok=True)
    op_path = output_dir / f"{output_prefix}_op_channel_attention.png"
    eis_path = output_dir / f"{output_prefix}_eis_channel_attention.png"
    op_temporal_path = output_dir / f"{output_prefix}_op_temporal_attention.png"
    eis_temporal_path = output_dir / f"{output_prefix}_eis_temporal_attention.png"

    _plot_heatmap(op_summary["matrix"], op_labels, class_labels, f"{title_prefix} OP Channel Attention", op_path)
    _plot_heatmap(eis_summary["matrix"], eis_labels, class_labels, f"{title_prefix} EIS Channel Attention", eis_path)
    _plot_heatmap(_mean_temporal_attention(outputs["op_temporal_attention"]), [f"t{i}" for i in range(outputs["op_temporal_attention"].shape[-1])], op_labels, f"{title_prefix} OP Temporal Attention", op_temporal_path)
    _plot_heatmap(_mean_temporal_attention(outputs["eis_temporal_attention"]), [f"t{i}" for i in range(outputs["eis_temporal_attention"].shape[-1])], eis_labels, f"{title_prefix} EIS Temporal Attention", eis_temporal_path)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output-prefix", default="public_ac_voltage")
    parser.add_argument("--title-prefix", default="Public AC Voltage")
    parser.add_argument("--op-channel-labels", default=DEFAULT_OP_CHANNEL_LABELS)
    parser.add_argument("--eis-channel-labels", default=DEFAULT_EIS_CHANNEL_LABELS)
...
def main(argv: list[str] | None = None) -> None:
    ...
    outputs = run_attention_plot(
        ...
        op_channel_labels=str(args.op_channel_labels),
        eis_channel_labels=str(args.eis_channel_labels),
    )
