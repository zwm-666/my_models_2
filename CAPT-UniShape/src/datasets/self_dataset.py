...
# Independent operating condition variables (pressure, temperature, etc.)
# These are orthogonal to x_op (stack time series) and x_eis (impedance shape).
# Selected via RF importance + ANOVA F-ratio + correlation deduplication.
COND_COLS: List[str] = [
    "进堆空压",          # 空气侧压力代表 (压力组)
    "出堆水温",          # 冷却系统温度代表 (温度组)
    "高压水泵转速FK",     # 冷却系统控制量代表 (控制组)
    "氢压差",            # 氢气侧压降 (独立)
    "空压差",            # 空气侧压降 (独立)
    "水温升",            # 冷却水温升 (独立)
    "FC空压机出口温度",    # 压缩热指标 (独立)
